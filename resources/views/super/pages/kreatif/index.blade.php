            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="button-items d-grid gap-2 d-md-flex justify-content-md-end">
                        <a href="{{ route('super-kegiatan-kreatif-create')}}" type="button"
                        ...
                        <table class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            <tbody>
                                @foreach ($kreatifs as $index => $kreatif)
                                    <tr>
                                        <td>{{ $index + $kreatifs->firstItem() }}</td>
                                        <td>{{ $kreatif->nama_warga }}</td>
                                        <td>{{ $kreatif->komponen }}</td>
                                        <td>{{ date('d-m-Y', strtotime($kreatif->waktu)) }}</td>
                                        <td>{{ $kreatif->tempat }}</td>
                                        <td>
                                            <a href="/super-kegiatan-kreatif-detail/{{ $kreatif->id }}"
